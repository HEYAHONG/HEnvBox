...
if [ ${HENVBOX_UNSUPPORTED} -ne 1 ]; then
	...
	for bin in $(find ${HENVBOX_LOCAL_ROOT_PATH_UNIX} -mindepth 2 -maxdepth 2 -type d -name bin 2>/dev/null); do
		export PATH=${PATH}:${bin}
	done

	#导入tools中的配置脚本
	if [ -x "${HENVBOX_ROOT_PATH}/tools/${HENVBOX_TYPE}/config.sh" ]; then
		. "${HENVBOX_ROOT_PATH}/tools/${HENVBOX_TYPE}/config.sh"
	fi

	if [ "${CONFIG_HENVBOX_TOOLS_PATH_PRIORITY_HIGH}" = "y" ]; then
