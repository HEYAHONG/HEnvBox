...
if [ ${HENVBOX_UNSUPPORTED} -ne 1 ]; then
	...
	if [ ${HENVBOX_UNSUPPORTED} -ne 0 ]; then
		exit 1
	fi
	#导入tools中的安装脚本
	if [ -x "${HENVBOX_ROOT_PATH}/tools/${HENVBOX_TYPE}/install.sh" ]; then
		. "${HENVBOX_ROOT_PATH}/tools/${HENVBOX_TYPE}/install.sh"
	fi
	...
	touch ${BASHRC_PATH}
	BASHRC_ENV_BLOCK_BEGIN=$(cat ${BASHRC_PATH} | grep -n "^#HEnvBox Block BEGIN$")
	BASHRC_ENV_BLOCK_END=$(cat ${BASHRC_PATH} | grep -n "^#HEnvBox Block END$")
	if [ -z "${BASHRC_ENV_BLOCK_BEGIN}" ]; then
		cat >>${BASHRC_PATH} <<-EOF
			#HEnvBox Block BEGIN
