...
for i in $(find ${SCRIPT_DIR} -name PreInstall.sh); do
	if [ -x $i ]; then
		echo execute $i
		. $i
	fi
...
if [ -n "${DISTRIB_CODENAME}" ]; then
	...
	for i in $(find ${SCRIPT_DIR} -name PreInstall.${DISTRIB_CODENAME}.sh); do
		if [ -x $i ]; then
			echo execute $i
			. $i
		fi
...
#安装相应软件包
for list_file in $(find ${SCRIPT_DIR} -name Packages.list); do
...
if [ -n "${DISTRIB_CODENAME}" ]; then
	#安装相应发行版软件包
	for list_file in $(find ${SCRIPT_DIR} -name Packages.${DISTRIB_CODENAME}.list); do
		echo install packages list ${list_file}
		set -e
		cat ${list_file} | xargs apt install -yyy
		set +e
	...
	SNAP_BIN=$(which snap)
	if [ -x ${SNAP_BIN} ]; then
		for list_file in $(find ${SCRIPT_DIR} -name "Snap.${DISTRIB_CODENAME}.list"); do
			echo install snap list ${list_file}
			for SNAP in $(cat ${list_file}); do
...
for i in $(find ${SCRIPT_DIR} -name PostInstall.sh); do
	if [ -x $i ]; then
		echo execute $i
		. $i
	fi
...
if [ -n "${DISTRIB_CODENAME}" ]; then
	...
	for i in $(find ${SCRIPT_DIR} -name PostInstall.${DISTRIB_CODENAME}.sh); do
		if [ -x $i ]; then
			echo execute $i
			. $i
		fi
