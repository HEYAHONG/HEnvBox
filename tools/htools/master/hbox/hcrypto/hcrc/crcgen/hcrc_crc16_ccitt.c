...
static uint16_t __hcrc_crc16_ccitt_table_data[]=
{
...
static uint16_t __hcrc_crc16_ccitt_table(uint8_t index)
{
    return __hcrc_crc16_ccitt_table_data[index];
};
uint16_t hcrc_crc16_ccitt_table_starts(void)
{
    return 0;
};
uint16_t hcrc_crc16_ccitt_table_update(uint16_t ctx,const uint8_t *data,size_t datalen)
{
    uint16_t crc=ctx;
    if(data!=NULL && datalen > 0)
    {
        for(size_t i=0; i<datalen; i++)
        {
            ...
            crc = (crc >> sizeof(current_data)*8) ^__hcrc_crc16_ccitt_table((crc ^ ((uint16_t)current_data)));
        }
    }
    return crc;
};
uint16_t hcrc_crc16_ccitt_table_finish(uint16_t ctx)
{
    return ctx^0;
};
uint16_t hcrc_crc16_ccitt_table_calculate(const uint8_t *data,size_t datalen)
{
    uint16_t ctx=hcrc_crc16_ccitt_table_starts();
    ctx=hcrc_crc16_ccitt_table_update(ctx,data,datalen);
    return hcrc_crc16_ccitt_table_finish(ctx);
};
bool    hcrc_crc16_ccitt_table_check(const uint8_t *data,size_t datalen,uint16_t check)
{
    return check==hcrc_crc16_ccitt_table_calculate(data,datalen);
};
