...
static uint32_t __hcrc_crc32_32c_table_data[]=
{
...
static uint32_t __hcrc_crc32_32c_table(uint8_t index)
{
    return __hcrc_crc32_32c_table_data[index];
};
...
uint32_t hcrc_crc32_32c_table_update(uint32_t ctx,const uint8_t *data,size_t datalen)
{
    ...
    if(data!=NULL && datalen > 0)
    {
        for(size_t i=0; i<datalen; i++)
        {
            ...
            crc = (crc >> sizeof(current_data)*8) ^__hcrc_crc32_32c_table((crc ^ ((uint32_t)current_data)));
        }
    }
    return crc;
};
...
{
    uint32_t ctx=hcrc_crc32_32c_table_starts();
    ctx=hcrc_crc32_32c_table_update(ctx,data,datalen);
    return hcrc_crc32_32c_table_finish(ctx);
};
bool    hcrc_crc32_32c_table_check(const uint8_t *data,size_t datalen,uint32_t check)
{
    return check==hcrc_crc32_32c_table_calculate(data,datalen);
};
