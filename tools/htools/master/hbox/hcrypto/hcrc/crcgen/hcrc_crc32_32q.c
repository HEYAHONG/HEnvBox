...
static uint32_t __hcrc_crc32_32q_table_data[]=
{
    0,0x814141ab,0x83c3c2fd,0x2828356,0x86c6c451,0x78785fa,0x50506ac,0x84444707,
    0x8cccc909,0xd8d88a2,0xf0f0bf4,0x8e4e4a5f,0xa0a0d58,0x8b4b4cf3,0x89c9cfa5,0x8888e0e,
    ...
    0x8888e0e0,0x9c9a14b,0xb4b221d,0x8a0a63b6,0xe4e24b1,0x8f0f651a,0x8d8de64c,0xccca7e7,
    0x44429e9,0x85056842,0x8787eb14,0x6c6aabf,0x8282edb8,0x3c3ac13,0x1412f45,0x80006eee,
    0
};
static uint32_t __hcrc_crc32_32q_table(uint8_t index)
{
    return __hcrc_crc32_32q_table_data[index];
};
uint32_t hcrc_crc32_32q_table_starts(void)
{
    return 0;
};
uint32_t hcrc_crc32_32q_table_update(uint32_t ctx,const uint8_t *data,size_t datalen)
{
    uint32_t crc=ctx;
    if(data!=NULL && datalen > 0)
    {
        for(size_t i=0; i<datalen; i++)
        {
            ...
            crc = (crc << sizeof(current_data)*8) ^ __hcrc_crc32_32q_table((crc>>((sizeof(crc)-sizeof(current_data))*8) ^ ((uint32_t)current_data)));
        }
    }
    return crc;
};
uint32_t hcrc_crc32_32q_table_finish(uint32_t ctx)
{
    return ctx^0;
};
uint32_t hcrc_crc32_32q_table_calculate(const uint8_t *data,size_t datalen)
{
    uint32_t ctx=hcrc_crc32_32q_table_starts();
    ctx=hcrc_crc32_32q_table_update(ctx,data,datalen);
    return hcrc_crc32_32q_table_finish(ctx);
};
bool    hcrc_crc32_32q_table_check(const uint8_t *data,size_t datalen,uint32_t check)
{
    return check==hcrc_crc32_32q_table_calculate(data,datalen);
};
