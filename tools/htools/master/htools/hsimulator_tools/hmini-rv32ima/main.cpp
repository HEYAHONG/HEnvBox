...
int main( int argc, char ** argv )
{
    ...
    for( i = 1; i < argc; i++ )
    {
        ...
        do
        {
            if( param[0] == '-' || param_continue )
            {
                ...
                {
                case 'm':
                    if( ++i < argc ) ram_amt = SimpleReadNumberInt( argv[i], ram_amt );
                    break;
                case 'c':
                    if( ++i < argc ) instct = SimpleReadNumberInt( argv[i], -1 );
                    break;
                case 'k':
                    if( ++i < argc ) kernel_command_line = argv[i];
                    break;
                case 'f':
                ...
                case 'd':
                    ...
                    break;
                case 't':
                    if( ++i < argc ) time_divisor = SimpleReadNumberInt( argv[i], 1 );
                    break;
                default:
                    if( param_continue )
                        param_continue = 0;
    ...
    if( show_help || image_file_name == 0 || time_divisor <= 0 )
    {
        ...
        if(banner!=NULL)
        {
            fprintf(stderr,"%s",(const char *)banner);
        }
        return 1;
    }
