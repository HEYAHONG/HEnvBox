...
#加载安装sh脚本
if [ -f "${HENVBOX_TOOLS_PATH}/install.sh" ]; then
...
for i in $(find . -name PreInstall.sh); do
	if [ -f $i ]; then
		echo execute $i
		. $i
	fi
...
#安装列表中的软件包
for list_file in $(find . -name Packages.list); do
...
for i in $(find . -name PostInstall.sh); do
	if [ -f $i ]; then
		echo execute $i
		. $i
	fi
...
#创建bat到msys2的入口
for list_file in $(find . -name BatToMsys2.list); do
